...
const MemberActivation = () => {
  ...
  const onSetMembershipType = (val: string) => setMembershipId(val);
  const onSetGender = (val: string) => setGender(val);
  const onSetDate = (val: Date | undefined) => setDateOfBirth(val);
  const onSetStartDate = (val: Date | undefined) => setStartDate(val);
        ...
        <form onSubmit={onSubmit}>
          <Tabs defaultValue="member_data" className="w-[400px] mx-auto">
            ...
            <TabsContent value="subscription_data">
              <p className="text-sm text-muted-foreground mb-4">Enter the chosen subscription model.</p>
